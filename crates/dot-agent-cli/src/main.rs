...
mod args;
use args::{
    ChannelAction, Cli, Commands, ConfigAction, HubAction, PluginAction, ProfileAction,
...
fn main() -> ExitCode {
    ...
    let result = match cli.command {
        ...
        Some(Commands::Plugin { action }) => handle_plugin(action),
...
fn handle_channel(action: ChannelAction, base_dir: &Path) -> Result<()> {
    ...
    match action {
        ...
        ChannelAction::AddPlugin { url, name } => {
            use dot_agent_core::PluginInstaller;
            ...
            let repo = if url.starts_with("https://github.com/") {
                ...
                // Assume it's already a repo like "owner/repo"
                url.clone()
            } else {
                return Err(DotAgentError::ConfigParseSimple {
                    message: format!("Invalid marketplace URL: {}", url),
                });
            };

            let marketplace_name = name
                .unwrap_or_else(|| repo.rsplit('/').next().unwrap_or("marketplace").to_string());

            println!();
            println!("Adding Claude Code Plugin Marketplace...");
            println!("  Name: {}", marketplace_name.cyan());
            println!("  Repo: {}", repo.dimmed());

            let installer = PluginInstaller::new()?;
            let path = installer.add_marketplace_github(&marketplace_name, &repo)?;

            println!();
            println!(
                "{} {}",
                "Added marketplace:".green(),
                marketplace_name.cyan()
            );
            println!("  Path: {}", path.display());

            // Also add as a channel
            let channel = Channel::claude_plugin_github(&marketplace_name, &repo);
            registry.add(channel)?;
            registry.save(base_dir)?;

            println!("  Channel: {}", "enabled".green());
        }
    }
...
fn handle_plugin(action: PluginAction) -> Result<()> {
    use dot_agent_core::{InstallScope, PluginInstaller};

    let installer = PluginInstaller::new()?;

    match action {
        PluginAction::Search { query, limit } => {
            let results = installer.search_plugins(&query)?;
            ...
            for (i, plugin) in results.iter().take(limit).enumerate() {
                let installed_badge = if plugin.installed.is_some() {
        ...
        PluginAction::List { marketplace } => {
            let plugins = installer.list_marketplace_plugins(&marketplace)?;

            if plugins.is_empty() {
                println!("No plugins found in marketplace: {}", marketplace);
                return Ok(());
            }

            println!();
            println!("Plugins in '{}':", marketplace.cyan().bold());
            println!();
            ...
            for plugin in &plugins {
                ...
                println!("  {}{}", plugin.name.cyan(), installed_badge);
                if let Some(desc) = &plugin.description {
                    println!("    {}", desc.dimmed());
                }
            }
        ...
        PluginAction::Installed => {
            let plugins = installer.list_installed_plugins()?;

            if plugins.is_empty() {
                println!("No plugins installed.");
                println!();
                println!("{}", "Search for plugins:".dimmed());
                println!("  {}", "dot-agent plugin search <query>".dimmed());
                return Ok(());
            }
            ...
            for (full_id, plugin) in &plugins {
                println!(
                    "  {} ({})",
                    full_id.cyan(),
                    plugin.scope.to_string().dimmed()
                );
                println!("    Version: {}", plugin.version);
                println!("    Path: {}", plugin.install_path);
                println!();
            }
        }
        PluginAction::Info { name, marketplace } => {
            let plugin = installer.get_plugin(&name, &marketplace)?;

            match plugin {
                Some(p) => {
                    println!();
                    println!("Plugin: {}", p.full_id.cyan().bold());
                    println!();
                    ...
                    if let Some(installed) = &p.installed {
                        println!();
                        println!("{}", "Installation:".green().bold());
                        println!("  Scope: {}", installed.scope);
                        println!("  Path: {}", installed.install_path);
                        println!("  Version: {}", installed.version);
                    }
                }
                None => {
                    println!("Plugin not found: {}@{}", name, marketplace);
                }
            }
        }
        PluginAction::Install {
            ...
            let install_scope = match scope.to_lowercase().as_str() {
                "user" => InstallScope::User,
                "project" => InstallScope::Project,
                "local" => InstallScope::Local,
                _ => {
                    return Err(DotAgentError::ConfigParseSimple {
                        message: format!("Invalid scope: {}. Use: user, project, local", scope),
                    });
                }
            };

            println!();
            println!("Installing plugin: {}@{}", name.cyan(), marketplace);
            println!("Scope: {}", scope);

            let installed = installer.install_plugin(&name, &marketplace, install_scope)?;

            let full_id = format!("{}@{}", name, marketplace);
            println!();
            println!("{} {}", "Installed:".green(), full_id.cyan());
            println!("  Path: {}", installed.install_path);
        }
        PluginAction::Uninstall { name, marketplace } => {
            println!();
            println!("Uninstalling plugin: {}@{}", name.cyan(), marketplace);

            installer.uninstall_plugin(&name, &marketplace)?;

            println!();
            println!("{} {}@{}", "Uninstalled:".red(), name, marketplace);
        }
        PluginAction::AddMarketplace {
            source,
            name,
            local,
        } => {
            let marketplace_name = name.unwrap_or_else(|| {
                source
                    .trim_end_matches('/')
                    .rsplit('/')
                    .next()
            ...
            println!();
            println!("Adding marketplace: {}", marketplace_name.cyan());

            let path = if local {
                installer.add_marketplace_local(&marketplace_name, &source)?
            } else if source.starts_with("https://github.com/") {
                let repo = source
                    .trim_start_matches("https://github.com/")
                    .trim_end_matches('/')
                    .trim_end_matches(".git");
                installer.add_marketplace_github(&marketplace_name, repo)?
            } else if source.contains('/') && !source.contains("://") {
                installer.add_marketplace_github(&marketplace_name, &source)?
            } else {
                installer.add_marketplace_url(&marketplace_name, &source)?
            };

            println!();
            println!("{} {}", "Added:".green(), marketplace_name.cyan());
            println!("  Path: {}", path.display());
        }
        PluginAction::RemoveMarketplace { name } => {
            println!();
            println!("Removing marketplace: {}", name.yellow());

            installer.remove_marketplace(&name)?;

            println!();
            println!("{} {}", "Removed:".red(), name);
        }
        PluginAction::ListMarketplaces => {
            let marketplaces = installer.list_marketplaces()?;

            if marketplaces.is_empty() {
                println!("No marketplaces registered.");
                println!();
                println!("{}", "Add a marketplace:".dimmed());
                println!(
                    "  {}",
                    "dot-agent plugin add-marketplace owner/repo".dimmed()
                );
                return Ok(());
            }

            println!();
            println!("{}", "Registered Marketplaces:".cyan().bold());
            println!();
        ...
        PluginAction::UpdateMarketplace { name } => {
            let marketplaces: Vec<String> = if let Some(n) = name {
                vec![n]
            } else {
                installer
                    .list_marketplaces()?
            ...
            println!();
            println!("Updating marketplaces...");

            for marketplace_name in &marketplaces {
                print!("  {} ... ", marketplace_name);
                match installer.update_marketplace(marketplace_name) {
...
fn handle_profile(action: ProfileAction, base_dir: &Path) -> Result<()> {
    ...
    match action {
        ...
        ProfileAction::Publish {
            ...
            use dot_agent_core::{
                InstallScope, ProfilePluginConfig, ProfilePublisher, VersionBump,
            };
            ...
            let mut config = ProfilePluginConfig {
            ...
            println!();
            println!("Publishing profile: {}", name.cyan());

            let result = if let Some(bump_type) = bump {
                let version_bump = match bump_type.to_lowercase().as_str() {
                    ...
                    _ => {
                        return Err(DotAgentError::ConfigParseSimple {
                            message: format!(
                                "Invalid bump type: {}. Use: patch, minor, major",
                                bump_type
                            ),
                        });
                    }
                };
                publisher.republish(&profile, &config, install_scope, version_bump)?
            } else {
                // Check if already published to determine version
                if let Ok(Some(installed)) = publisher.get_published(&name) {
                    config.version = installed.version;
                }
                publisher.publish(&profile, &config, install_scope)?
            };

            println!();
            println!("{} {}", "Published:".green(), result.full_id.cyan());
            println!("  Version: {}", result.version);
            println!("  Path: {}", result.install_path.display());
            println!("  Files: {}", result.files_copied);

            if result.rules_copied {
                println!("  Rules: copied to ~/.claude/rules/");
            }
            if result.claude_md_copied {
                println!("  CLAUDE.md: copied to ~/.claude/CLAUDE.md");
            }

            println!();
            println!(
                "{}",
                "Profile is now available as a Claude Code plugin.".green()
            );
        }
        ProfileAction::Unpublish { name, force } => {
            use dot_agent_core::ProfilePublisher;

            let publisher = ProfilePublisher::new()?;

            // Check if published
            if !publisher.is_published(&name)? {
                return Err(DotAgentError::ConfigParseSimple {
                    message: format!("Profile '{}' is not published", name),
                });
            }

            if !force {
                println!();
                println!("Unpublish profile '{}'?", name.yellow());
                println!("This will remove the plugin from Claude Code's cache.");
                println!();
                print!("Type 'yes' to confirm: ");
                io::stdout().flush()?;
            ...
            println!();
            println!("{} {}", "Unpublished:".red(), name);
        }
        ProfileAction::Published => {
            use dot_agent_core::ProfilePublisher;

            let publisher = ProfilePublisher::new()?;
            let published = publisher.list_published()?;

            if published.is_empty() {
                println!("No published profiles.");
                println!();
                println!("{}", "Publish a profile:".dimmed());
                println!("  {}", "dot-agent profile publish <name>".dimmed());
                return Ok(());
            }

            println!();
            println!("{}", "Published Profiles:".cyan().bold());
            println!();

            for (full_id, plugin) in &published {
                let name = full_id.split('@').next().unwrap_or(full_id);
                println!("  {} ({})", name.cyan(), plugin.scope.to_string().dimmed());
                println!("    Version: {}", plugin.version);
                println!("    Path: {}", plugin.install_path);
                println!();
            }
